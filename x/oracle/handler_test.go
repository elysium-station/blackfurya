...
func TestOracleFilters(t *testing.T) {
	...
	salt = "2"
	hash = types.GetAggregateVoteHash(salt, denom1ExchangeRateStr, sdk.ValAddress(nonValidatorAddr))
...
func TestFeederDelegation(t *testing.T) {
	...
	voteMsg = types.NewMsgAggregateExchangeRateVote(salt, denom1ExchangeRateStr, keeper.Addrs[1], keeper.ValAddrs[0])
	_, err = h(input.Ctx.WithBlockHeight(1), voteMsg)
	require.Error(t, err)
	...
	voteMsg = types.NewMsgAggregateExchangeRateVote(salt, denom1ExchangeRateStr, keeper.Addrs[2], keeper.ValAddrs[0])
	_, err = h(input.Ctx.WithBlockHeight(1), voteMsg)
	require.Error(t, err)
