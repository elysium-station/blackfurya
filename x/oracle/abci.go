...
func EndBlocker(ctx sdk.Context, k keeper.Keeper) {
	...
	if blackfury.IsPeriodLastBlock(ctx, params.VotePeriod) {
		...
		if referenceBlackDebt := PickReferenceBlackDebt(ctx, k, voteTargets, voteMap); referenceBlackDebt != "" {
			// make voteMap of Reference BlackDebt to calculate cross exchange rates
			ballotRM := voteMap[referenceBlackDebt]
